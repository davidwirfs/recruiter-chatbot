...
| `FALLBACK_API_KEY` | _(empty → fallback disabled)_ | **Best-effort, account-dependent.** Free OpenRouter key from [openrouter.ai/keys](https://openrouter.ai/keys). When set, the configured fallback provider is used whenever Gemini exhausts its retries. **Caveat:** in v0.6.x trials from this Space, every free-tier provider attempted (Groq × 3, OpenRouter free, Cerebras × 2) returned `403 Forbidden` or `429` — per-account model gating and HF Space IP flagging block most free tiers. Topping up $5 of OpenRouter credit reliably unlocks the slot (raises daily limit 50 → 1000). See [docs/2026-05-11-CHANGELOG.md § 0.6.6](docs/2026-05-11-CHANGELOG.md) for the full trial log. |
